...
for i in {0..8887}; do
  ...
  if [ "$count" -gt 1 ]; then
    ...
    ffmpeg -r 15 -start_number 0 -i "${i}_%05d.png" -i output.wav -map 0:v -map 1:a -c:v libx264 -crf 12 -pix_fmt yuv420p ../../videos/"${i}".mp4
    # clean up so if we re-run it doesn't get mixed again
    rm output.wav
  elif [ "$count" == 1 ]; then
    # only 1 wav file found, sox can't combine
    name=$(find ./*.wav)
    ffmpeg -r 15 -start_number 0 -i "${i}_%05d.png" -i "$name" -map 0:v -map 1:a -c:v libx264 -crf 12 -pix_fmt yuv420p ../../videos/"${i}".mp4
  else
    # no wav files found
